/*!
 * Photo Sphere Viewer / Virtual Tour Plugin 5.11.4
...
var ArrowsRenderer = class extends import_core3.AbstractComponent {
  ...
  handleEvent(e) {
    switch (e.type) {
      ...
      case "mouseenter": {
        const link = this.__getTargetLink(import_core3.utils.getEventTarget(e));
        if (link) {
      ...
      case "mousemove": {
        const link = this.__getTargetLink(import_core3.utils.getEventTarget(e), true);
        if (link) {
...
var AbstractDatasource = class {
  ...
  checkLink(node, link) {
    ...
    if (link.nodeId === node.id) {
      throw new import_core4.PSVError(`Node ${node.id} links to itself`);
    }
...
var ClientSideDatasource = class extends AbstractDatasource {
  async loadNode(nodeId) {
    if (this.nodes[nodeId]) {
      ...
      throw new import_core5.PSVError(`Node ${nodeId} not found`);
    }
  }
  setNodes(rawNodes) {
    if (!rawNodes?.length) {
  ...
  updateNode(rawNode) {
    if (!rawNode.id) {
      throw new import_core5.PSVError("No id given for node");
    }
    const node = this.nodes[rawNode.id];
    if (!node) {
      throw new import_core5.PSVError(`Node ${rawNode.id} does not exist`);
    }
  ...
  setCurrentNode(nodeId, options, fromLink) {
    ...
    return Promise.resolve(this.state.preload[nodeId]).then(() => {
      ...
      if ("fadeIn" in transitionOptions) {
        import_core7.utils.logWarn('VirtualTourTransitionOptions.fadeIn is deprecated, use "effect" instead.');
...
VirtualTourPlugin.VERSION = "5.11.4";
